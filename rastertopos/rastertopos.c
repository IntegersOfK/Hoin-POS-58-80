...
static void ejectCashDrawer(int no, int beforeOrAfter);
...
Setup(void)
{
	...
	if(cashDrawerSetting == EJECT_CASH_DRAWER1_BEFORE_PRINT){
		...
		ejectCashDrawer(3, 0);
	}else if(cashDrawerSetting == EJECT_CASH_DRAWER1_AFTER_PRINT){
		ejectCashDrawer(1, 1);
...
Shutdown(void)
{
	...
	if(cashDrawerSetting == EJECT_CASH_DRAWER1_AFTER_PRINT ||
			cashDrawerSetting == EJECT_CASH_DRAWER2_AFTER_PRINT ||
			cashDrawerSetting == EJECT_CASH_DRAWER12_AFTER_PRINT) {
		char buf[4];
     ...
     char *argv[])	/* I - Command-line arguments */
{
	...
	*/
	...
	if( ppd == NULL ){
		fputs("DEBUG: ppd   is  NULL\n",stderr);
	}else{
		fputs("DEBUG: ppd  is not NULL\n",stderr);
	}
	
	choice = ppdFindMarkedChoice(ppd, "CashDrawer");    //钱箱
	if( choice != NULL ){
		cashDrawerSetting = atoi(choice->choice);   
	}	
	choice = ppdFindMarkedChoice(ppd, "BlankSpace");    //留白
	if( choice != NULL ){
	    blankSpaceSetting = atoi(choice->choice);
	}
	choice = ppdFindMarkedChoice(ppd, "FeedDist");    //走纸
	if( choice != NULL ){
	    feedDistSetting = atoi(choice->choice);
	}
	choice = ppdFindMarkedChoice(ppd, "Cutting");    //切纸
	if( choice != NULL ){
	    cuttingSetting = atoi(choice->choice);
	}
	choice = ppdFindMarkedChoice(ppd, "Beeper");    //蜂鸣器鸣叫
	if( choice != NULL ){
	    beeperSetting = atoi(choice->choice);
	}
	choice = ppdFindMarkedChoice(ppd, "NVLogo");    //打印logo
	if( choice != NULL ){
